...
(deftest llm-token-spend-by-agent
  (let [collector (telemetry/create-collector {:enabled true})
        ...
        content (llm-instrumented/complete-with-telemetry! collector
                                                           provider
...
(deftest complete-with-telemetry-captures-tool-calls-and-real-usage
  (let [events (atom [])
        ...
        content (llm-instrumented/complete-with-telemetry! (telemetry/create-collector {:enabled true})
                                                           (->ToolCallProvider)
...
(deftest multi-observer-fans-out-and-is-best-effort
  (let [events (atom [])
        metrics (atom [])
        observer (telemetry-observer/->MultiObserver [(->FailingObserver)
                                                      (->CountingObserver events metrics)]
                                                     true)]
    (telemetry-observer/record-event! observer {:event-type :turn/start :payload {:id "t1"}})
    (telemetry-observer/record-metric! observer {:metric-type :queue-depth :value 2})
