...
**🟡 pool-config-not-forwarded — MEDIUM.** `create-datasource` (`common.clj:84`) reads `:maximum-pool-size`/`:minimum-idle`/`:connection-timeout-ms` (defaults 4/1/30000), but nothing in the config path forwards them, so the pool is permanently **4 connections**. Combined with `runtime-health`'s N+1 (below) and autocommit readers, four concurrent readers can starve a `with-transaction` writer for up to 30s. **Fix:** add the keys to `:storage :sqlite` defaults and pass through; raise the default (WAL supports many readers). **Status:** ✅ fixed in `15b4fa2`; defaults are 8/2/30000 with env overrides.

**🟡 runtime-health-n+1 — MEDIUM.** `runtime/core.clj:520` loads up to 1000 runs then fires one `list-agent-run-commands` per run, each opening/closing a pooled connection. A liveness probe can fan out ~1000 queries against a 4-connection pool. **Fix:** one `select run_id, count(*) … group by run_id` (or a single total). **Status:** ✅ fixed in `15b4fa2`; health uses a grouped pending-command-count query.

**🟡 decide-approval-no-pending-guard — MEDIUM.** `decide-tool-approval` SQL (`resources/.../tools.sql:17`) is `update … where id=:id` with **no `and status='pending'`** and no CAS, on a plain `with-connection`. A denied/expired approval can be silently flipped to approved; two decisions race last-writer-wins. Approvals gate shell/fs-writes. **Fix:** add `and status='pending'` (and optional expiry predicate); treat 0-row update as conflict (the existing `(when (zero? updated) (throw …))` then surfaces it). **Status:** ✅ fixed in `190db38`; stale decisions are conflicts and API returns 409.
