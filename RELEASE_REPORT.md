...
**Verification (2026-05-30):** §5.5 Medium fixes are done for SQLite persistence: `pool-config-not-forwarded`, `runtime-health-n+1`, `decide-approval-no-pending-guard`, and `migration-checksum-cosmetic` are fixed. Focused SQLite/config suite: **40 tests, 164 assertions, 0 failures, 0 errors** (`agent.persistence.sqlite-test`, `agent.config-test`). Remaining open item in this section: `datahike-full-edge-scan`.

**🟡 migration-checksum-cosmetic — MEDIUM.** Each migration used to carry a hand-written literal `:checksum`; `record-migration-meta!` stored that literal and `verify-migration-checksums!` compared it to the same literal, so SQL edits were invisible. **Status:** ✅ fixed in this commit; descriptors no longer carry `:checksum`, `record-migration-meta!` and verify derive `sha256(str version up down)[:16]`, and legacy literal checksums are backfilled before verification so existing DBs do not false-positive or destructively reset. Regression coverage verifies edited applied SQL now raises `:migration-drift` and legacy literals backfill safely.
...
**🟢 Lower:** ✅ `journal-mode-bypasses-busy-timeout` fixed in this commit: `apply-journal-mode!` now goes through `with-connection`, so configured busy-timeout and SQLite retry wrap the journal PRAGMA. ✅ `select-value-broken` fixed in this commit: `select-value` now reads column 1 directly from its own `ResultSet`, with regression coverage for `SELECT 42 AS n`. ⬜ `datahike-full-edge-scan` remains open: `query-edges` still pulls all edges, and neighborhood/path queries filter in memory (`datahike.clj:358-390`).
...
- **Magic numbers** — `max-steps 6` and `tool-output-max-chars 8000` buried in `:or` destructuring (`loop.clj:253`), queue buffer sizes, `temperature 0.2`, `max_tokens 1024`. Promote remaining literals to named `def`s / config. **Note: `max-steps 6` is low for a coding agent** — confirm `chat.clj` overrides it; if not, that caps every turn at 6 tool iterations.
