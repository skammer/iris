...
- **`run!` god function / terminal-result duplication** — **Status:** ✅ fixed as originally written. `terminal-result` is centralized (`runtime/loop.clj:230-245`) and `fatal-guardrail!` uses it (`runtime/loop.clj:247-257`); terminal branches now call the helper instead of duplicating 7-key maps. Residual concern: `run!` is still large and still worth a later structural split, but the duplicated terminal-contract bug is gone.
...
- **Provider copy-paste** (`ollama.clj` ↔ `openai_compatible.clj`) — **Status:** ⬜ still valid. There is still no `agent.llm.providers.common`; both providers define their own `trim-trailing-slash`, `post-json`, `checked-response`, stream channel/thread skeleton, and structured-stream gate. **Fix:** extract shared HTTP/retry/body-close/stream scaffolding, keeping provider-specific payload parsing local.
- **Trivial-util duplication** — **Status:** ⬜ still valid, with lower current count than the original note. `duration-ms` remains in planner/runtime-tools/telemetry; cancellation helpers are duplicated in `runtime/loop.clj` and `runtime/tools.clj`; event/result string helpers are duplicated between loop/tools; timestamp helpers remain scattered. **Fix:** small `agent.util.time`, `agent.runtime.cancel`, and shared event/result-content helpers.
- **`execute-tool` repeated end-event maps** — **Status:** ⬜ still valid. `tools.core/execute-tool` still emits blocked/approval/success/failure maps inline (`tools/core.clj:333-399`); direct success/failure event payloads still omit `:duration-ms` even though duration is computed for hooks. **Fix:** `duration-ms` helper + `emit-tool-end!` with consistent payload shape.
- **Magic numbers** — **Status:** 🟨 partially stale, partially valid. `max-steps` is config-backed (`chat/turn.clj:143-145`, `config.clj`, `default.edn`) and chat regression coverage confirms override, so it is not an unconfigurable 6-step cap. Still valid: `runtime/loop.clj` keeps fallback defaults `max-steps 6` / `tool-output-max-chars 8000`, channel/broker buffer sizes remain scattered literals, and LLM default temperature/max-tokens are config constants but duplicated between `config.clj` and `resources/config/default.edn`. **Fix:** named defaults + one config source for mirrored defaults.
