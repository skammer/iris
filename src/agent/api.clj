...
(defn- handler-map
  ...
  {:health (fn [r] (health/handle (sys) r))
   ...
   :ui-cron-status (fn [r] (ui/cron-status (sys) r))
   :ui-cron-job-detail (fn [r] (ui/cron-job-detail (sys) r (path-param r :job-id)))
   :ui-cron-run-detail (fn [r] (ui/cron-run-detail (sys) r (path-param r :run-id)))
   ...
   :ui-chat-tool-detail (fn [r] (ui/chat-tool-detail (sys) r))
   :ui-chat-stop (fn [r] (ui/chat-stop (sys) r))
   :ui-events (fn [r] (ui/events (sys) r))
   :ui-logs (fn [r] (ui/logs (sys) r))
   :ui-log-detail (fn [r] (ui/log-detail (sys) r
                                         (path-param r :source)
                                         (path-param r :entry-id)))
   :ui-magi (fn [r] (ui/magi (sys) r))
   :ui-magi-detail (fn [r] (ui/magi-detail (sys) r (path-param r :event-id)))
   :ui-events-live (fn [r] (ui/events-live-response (sys) r))
   :ui-memory (fn [r] (ui/memory-workspace (sys) r))
   :ui-memory-vault-detail (fn [r] (ui/memory-vault-detail (sys) r (path-param r :note-id)))
   :ui-memory-update-detail (fn [r] (ui/memory-update-detail (sys) r (path-param r :update-id)))
   ...
   :ui-memory-proposal-decision (fn [r] (ui/memory-proposal-decision (sys) r))
   ...
   :ui-tool-approvals-status (fn [r] (ui/tool-approvals-status (sys) r))
   ...
   :set-session-project (fn [r] (sessions/set-project (sys) r (path-param r :session-id)))
