# Obsidian Vault Index

## Research
- [[core-async-flow]] - Flow-based programming for agents
- [[llm-integration]] - LiteLLM-clj and custom approaches
- [[existing-agents]] - Analysis of claw0, pi-mono, moltis, ironclaw
- [[knowledge-graphs]] - Oxford Semantic/RDFox, Noumenon

## Architecture
- [[codebase-map]] - Full namespace map, layer diagram, flows (June 2026 review)
- [[a2a-http-tasks]] - Async HTTP task API and A2A compatibility notes
- [[configuration-observability]] - Config overlay order and OTel export
- [[session-entries-unification]] - Deferred design: single source of truth for messages
- [[design-principles]] - Core architectural decisions
- [[component-structure]] - Module organization
- [[security-model]] - Sandboxing and authentication
- [[multi-agent-coordination]] - Orchestrator/worker patterns

## Implementation
- [[agent-loop]] - Core reasoning engine
- [[memory-system]] - Memory strategies
- [[tool-execution]] - Sandboxed execution framework
- [[knowledge-graph-integration]] - Graph-based reasoning
- [[market-based-allocation]] - Economic task allocation

## Best Practices
- [[clojure-patterns]] - Idiomatic Clojure for agents
- [[testing-strategies]] - Testing approaches
- [[performance-optimization]] - Optimization techniques
- [[deployment-patterns]] - Production deployment

## Decision Logs
- [[refactoring-2026-06-findings]] - Deep-review findings (bugs, dead code, duplication)
- [[refactoring-2026-06-plan]] - Phased refactoring plan + decisions
- [[archive/MEMORY_REFACTORING_PLAN]] - Archived Iris memory refactoring plan
- [[archive/REFACTORING_PLAN]] - Archived June 2026 refactoring sweep log
- [[archive/REPORT]] - Archived May 2026 remaining-issues report
- [[archive/RELEASE_REPORT]] - Archived May 2026 release review
- [[decisions-2026]] - Architectural decisions
- [[issue-tracking]] - Problems and solutions
- [[future-considerations]] - Future enhancements

## References
- [[external-resources]] - Documentation and tools
- [[glossary]] - Terminology definitions
- [[cheatsheets]] - Quick reference guides

## Tags
#research #architecture #implementation #best-practice #decision #issue #future #reference
